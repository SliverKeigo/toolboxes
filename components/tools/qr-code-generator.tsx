...
import { Button, Input, Textarea, Card, CardBody, CardHeader, CardFooter, RadioGroup, Radio, Divider, Tabs, Tab } from "@heroui/react"
...
export default function QRCodeGenerator() {
  ...
  useEffect(() => {
    if (qrStyle === "gradient" && qrRef.current) {
      ...
      // 获取渐变图像数据
      const gradientData = tempCtx.getImageData(0, 0, canvas.width, canvas.height).data
      ...
      <Card>
        ...
        <CardBody className="flex flex-col gap-6">
          <div>
            ...
            <Tabs aria-label="输入类型" variant="bordered" color="primary">
              <Tab key="text" title="文本" onClick={() => setInputType("text")} />
              <Tab key="url" title="网址" onClick={() => setInputType("url")} />
              <Tab key="file" title="文件" onClick={() => setInputType("file")} />
              <Tab key="image" title="图片" onClick={() => setInputType("image")} />
              <Tab key="audio" title="音频" onClick={() => setInputType("audio")} />
              <Tab key="video" title="视频" onClick={() => setInputType("video")} />
            </Tabs>
          </div>
          ...
          <div>
            ...
            {inputType === "text" || inputType === "url" ? (
              <Textarea
                ...
                placeholder={inputType === "url" ? "请输入网址..." : "请输入文本..."}
              />
            ) : (
              <div className="flex flex-col gap-2">
                ...
                {fileList.length > 0 && <span className="text-sm">{fileList[0].name}</span>}
              </div>
            )}
          </div>
            ...
            <div className="mt-4">
              {qrStyle === "solid" ? (
                <div className="flex gap-4">
                  <div>
                    <label className="block mb-1">前景色</label>
                    <input
                      type="color"
                      value={fgColor}
                  ...
                  <div>
                    <label className="block mb-1">背景色</label>
                    <input
                      type="color"
                      value={bgColor}
                ...
                <div className="flex flex-wrap gap-4">
                  <div>
                    <label className="block mb-1">起始色</label>
                    <input
                      type="color"
                      value={gradientStart}
                  ...
                  <div>
                    <label className="block mb-1">结束色</label>
                    <input
                      type="color"
                      value={gradientEnd}
                  ...
                  <div>
                    <label className="block mb-1">渐变方向</label>
                    <select 
          ...
          <div>
            <Button
              ...
              isDisabled={inputType === "file" || inputType === "image" || inputType === "audio" || inputType === "video"}
            >
              生成二维码
            </Button>
          </div>
